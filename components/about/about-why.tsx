import { Hammer, Gem, Heart, Award } from "lucide-react"
...
const reasons = [
  ...
  },
  {
    ...
    description: "Built with Pride, Precision, and Integrity.",
    content:
      "With years of hands-on experience, our skilled craftspeople bring precision and pride to every cut, joint, and finish. Our cabinetry is handcrafted right here in Sarasota, Florida, with a focus on clean lines, smooth operation, and impeccable detail.",
  },
]
    ...
    <section className="w-full bg-slate-50 py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6 md:px-16">
        ...
        <div className="flex flex-col items-center text-center gap-5 mb-16">
          ...
          <h2 className="text-slate-900 font-bold text-4xl md:text-5xl tracking-tight max-w-2xl leading-tight">
            Why Choose{" "}
            <span className="text-amber-400">Cabinetry by Wettach?</span>
          </h2>
          ...
          {reasons.map((item) => {
            ...
            return (
              <div
                key={item.title}
                className={[
                  "relative flex flex-col gap-5 p-8 rounded-sm border transition-all duration-300 group hover:-translate-y-1",
                  item.featured
                    ? "bg-slate-900 border-slate-900 shadow-2xl shadow-slate-900/20"
                    : "bg-white border-slate-100 hover:border-slate-200 hover:shadow-lg hover:shadow-slate-100/80",
                ].join(" ")}
              >
                {/* Icon */}
                <div
                ...
                <div
                  className={[
                    ...
                    item.featured ? "bg-amber-400" : "bg-amber-400 group-hover:w-14",
                  ].join(" ")}
                />
